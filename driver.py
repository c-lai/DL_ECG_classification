...
from run_12ECG_classifier_DL import load_12ECG_model, run_12ECG_classifier
import load

def load_challenge_data(filename, lead):
    # x = loadmat(filename)
    # data = np.asarray(x['val'], dtype=np.float64)
    ecg = np.reshape(load.load_ecg(filename)[lead - 1, :], (1, -1))
    ecg_mean = np.mean(ecg).astype(np.float32)
    ecg_std = np.std(ecg).astype(np.float32)
    data = (ecg - ecg_mean) / ecg_std
...
if __name__ == '__main__':
    ...
    for i, f in enumerate(input_files):
        ...
        data,header_data = load_challenge_data(tmp_input_file, lead)
